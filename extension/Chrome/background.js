...
chrome.webRequest.onBeforeRequest.addListener(
  (details) => {
    try {
      ...
      const bodyText = decodeRequestBody(details.requestBody.raw);
      ...
      console.error("DDB roll listener error:", error);
    }
  },
  {
    urls: ["https://api.dndbeyond.com/bi/events"],
...
function processDndBeyondPayload(payload) {
  ...
  for (const entry of entries) {
    ...
    if (
      detail.MESSAGE_NAME !== "GAME_ACTION" ||
      detail.ACTION_TAKEN !== "dice_roll"
    ) {
      continue;
    }

    const actions = detail.ACTION_DESCRIPTORS;
...
function processDiceAction(action) {
  const d20Results = action?.RESULTS?.D20;

  if (!Array.isArray(d20Results) || d20Results.length === 0) {
    return;
  }

  const rollId = action.ROLL_ID;

  if (rollId && processedRollIds.has(rollId)) {
    console.log("Ignoring duplicate DDB roll:", rollId);
  ...
  const trackerRolls = d20Results
    .filter((value) => Number.isInteger(value) && value >= 1 && value <= 20)
    .map((value) => ({
      value,
      type: value === 20 ? "nat20" : value === 1 ? "nat1" : "normal",
    }));
  ...
  console.log("🎲 D&D Beyond roll detected", {
